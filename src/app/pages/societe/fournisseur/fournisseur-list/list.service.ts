...
function matches(country: ListModel, term: string, pipe: PipeTransform) {
  return country.status.toLowerCase().includes(term.toLowerCase())
    ...
    || country.commune.toLowerCase().includes(term.toLowerCase())
    || country.categories.toLowerCase().includes(term.toLowerCase())
...
export class ListService {
  ...
  get product() { return this.suppliers; }
  ...
  private _search(): Observable<SearchResult> {
    ...
    // 5. Rate Filter
    if (this.productRate) {
      countries = countries.filter(country => country.name >= this.productRate);
    }
    else {
      countries = countries;
    }
