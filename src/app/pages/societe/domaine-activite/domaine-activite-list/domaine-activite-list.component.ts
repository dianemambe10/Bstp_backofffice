...
@Component({
  selector: 'app-domaine-activite-list',
  templateUrl: './domaine-activite-list.component.html',
  styleUrls: ['./domaine-activite-list.component.scss'],
  providers: [ListService, DecimalPipe]
})
export class DomaineActiviteListComponent {
  // bread crumb items
  ...
  ngOnInit(): void {
    ...
    setTimeout(() => {
     ...
     // this.domaineActiviteService.getData().subscribe()
  ...
  addBuyer(e: Event){
    ...
    e.preventDefault()
    this.router.navigate(['../societe/domaines-activites/nouveau']);
  ...
  confirmDelete() {
    if (this.deleteID) {
      this.domaineActiviteService.delete(this.deleteID).subscribe({
        next: (res)=> {

          this.toastService.success('La suppression d\' un domaine d\'activité a été effectué avec success', 'Succèss',{
            timeOut: 3000,
          })
          this.service.domaines = this.service.domaines.filter((product: any) => {
