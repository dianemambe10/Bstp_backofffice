...
function matches(country: ListModel, term: string, pipe: PipeTransform) {
  return country.reference?.toLowerCase().includes(term.toLowerCase())
    ...
    || country.payment_mode.name?.toLowerCase().includes(term.toLowerCase())
    || country.payment_mode.name?.toLowerCase().includes(term.toLowerCase())
    || country.transaction_time.toLowerCase().includes(term.toLowerCase())
    ;
}
...
export class ListService {
  ...
  user = [];
  abonnements: any | undefined;
  abonnements_back: any | undefined;
  Abonnements$: any;
  abonnements$: any;
  constructor(private pipe: DecimalPipe, private  abonnementService : AbonnementService) {
    this._search$.pipe(
  ...
  retrieveAbonnement(): void {
    this.abonnementService.getData()
      .subscribe({
        next: (data) => {
          this.abonnements = data;
          this.abonnements_back = data;
        },
  ...
  get abonnement() { return this.abonnements; }
